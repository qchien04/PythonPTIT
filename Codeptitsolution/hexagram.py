...
for t in range(int(input())):
    ...
    for i in range(sz):
        ...
        vs = [False]*12
        A[0], A[1], A[2] = arr[i]
        vs[A[0]], vs[A[1]], vs[A[2]] = True, True, True
        for j in range(i):
            if (not vs[arr[j][0]]) and (not vs[arr[j][1]]) and (not vs[arr[j][2]]):
                B[0], B[1], B[2] = arr[j]
                check = [False]*12
                check[A[0]], check[A[1]], check[A[2]] = True, True, True
                check[B[0]], check[B[1]], check[B[2]] = True, True, True
                cnt = 0
                for k in range(1, 12):
                    if not check[k]:
                        C[cnt] = k
